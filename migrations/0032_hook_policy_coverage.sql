CREATE TRIGGER coverage_hook_policy_insert AFTER INSERT ON operations
WHEN NEW.kind='hookrelay.subscription.policy'
BEGIN
...
CREATE TRIGGER coverage_hook_policy_update AFTER UPDATE ON operations
WHEN NEW.kind='hookrelay.subscription.policy'
BEGIN
  INSERT INTO operational_coverage_epochs(workspace_id,connection_id,generation)
    SELECT NEW.workspace_id,json_extract(input_json,'$.connectionId'),1 FROM action_plans WHERE id=NEW.plan_id
