...
describe("Production Access identity", () => {
  ...
  it("derives bearer authority from a live stored credential rather than its prefix", async () => {
    const { resolve } = fixture();
    const bearer = HQ_CREDENTIAL_PREFIX.PUBLISHER + "synthetic";
    ...
    const principal = await resolve(req, configured);
    ...
    expect(principal.sourceId).toBeUndefined();
    expect(principal.access).toBeUndefined();
