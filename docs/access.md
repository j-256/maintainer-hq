...
### Machine credential prefixes

HQ-issued bearer credentials use stable prefixes to identify their credential class:

| Prefix | Credential class |
| --- | --- |
| `hqa_` | Automation |
| `hqp_` | Local publisher |

Automation authority comes from the stored workspace and owner binding, live membership, expiry and revocation state, fixed profile, and scopes. Publisher authority comes from the stored workspace, owner, and source binding, live membership, expiry and revocation state, and enrolled repository scope.

A prefix is a recognition marker only. It grants no authority and must never replace digest lookup or live authorization. Prefix meanings are compatibility-sensitive; do not repurpose an existing prefix for another credential class.
